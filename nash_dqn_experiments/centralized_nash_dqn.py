...
class NashDQNAgent:
    ...
    def create_model(self, input_layer_size, action_space_size):
        ...
        model.load_weights('./models/weights/speaker_weights.h5') if self.is_speaker else model.load_weights('./models/weights/listener_weights.h5')
        return model
...
class JointCritic:
    ...
    def create_model(self, input_layer_size, action_space_size):
        ...
        model.add(Dense(action_space_size, activation = 'linear'))
        model.compile(loss="mse", optimizer=Adam(learning_rate=0.001), metrics=['accuracy'])
        model.load_weights('./models/weights/critic_weights.h5')
        return model
